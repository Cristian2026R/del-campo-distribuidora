...
def productos_page():
    ...
    with st.expander("➕ Agregar producto", expanded=False):
        ...
        with c1: nombre=st.text_input("Nombre producto nuevo", key="prod_add_nombre"); categoria=st.text_input("Categoría", value="General", key="prod_add_categoria")
        with c2: pu=st.number_input("Precio unidad",0.0,step=100.0, key="prod_add_precio_unidad"); pk=st.number_input("Precio kg",0.0,step=100.0, key="prod_add_precio_kg")
        with c3: cu=st.number_input("Costo unidad",0.0,step=100.0, key="prod_add_costo_unidad"); ck=st.number_input("Costo kg/litro",0.0,step=100.0, key="prod_add_costo_kg"); stock=st.number_input("Stock real inicial",0.0,step=1.0, key="prod_add_stock")
        unidad=st.selectbox("Unidad stock",["kg","litro","unidad","bolsa","caja","bulto"], key="prod_add_unidad_stock")
        cont_sugerido, med_sugerida = infer_packaging_from_name(nombre)
        c4,c5=st.columns(2)
        with c4: contenido=st.number_input("Contenido por unidad", min_value=0.001, value=float(cont_sugerido), step=0.5, help="Ej: Aceite X8Lts = 8. Harina 25Kg = 25.", key="prod_add_contenido")
        with c5: medida=st.selectbox("Medida contenido",["unidad","litro","kg"], index=["unidad","litro","kg"].index(med_sugerida) if med_sugerida in ["unidad","litro","kg"] else 0, key="prod_add_medida_contenido")
    ...
    ids=id_label(df)
    pid=st.selectbox("Editar / eliminar producto", list(ids.keys()), format_func=lambda x: ids.get(str(x),str(x)), key="prod_edit_select")
    pid_int=selected_int(pid)
    row=df[df.id==pid_int].iloc[0]
    c1,c2,c3=st.columns(3)
    with c1: en=st.text_input("Nombre", value=row.nombre, key=f"prod_edit_nombre_{pid_int}"); ec=st.text_input("Categoría", value=row.categoria, key=f"prod_edit_categoria_{pid_int}")
    with c2: epu=st.number_input("Precio unidad editable", value=float(row.precio_unidad), step=100.0, key=f"prod_edit_precio_unidad_{pid_int}"); epk=st.number_input("Precio kg editable", value=float(row.precio_kg), step=100.0, key=f"prod_edit_precio_kg_{pid_int}")
    with c3: ecu=st.number_input("Costo unidad editable", value=float(row.costo_unidad), step=100.0, key=f"prod_edit_costo_unidad_{pid_int}"); eck=st.number_input("Costo kg/litro editable", value=float(row.costo_kg), step=100.0, key=f"prod_edit_costo_kg_{pid_int}"); estock=st.number_input("Stock real editable", value=float(row.stock), step=1.0, key=f"prod_edit_stock_{pid_int}")
    eun=st.selectbox("Unidad stock", ["kg","litro","unidad","bolsa","caja","bulto"], index=["kg","litro","unidad","bolsa","caja","bulto"].index(row.unidad_stock) if row.unidad_stock in ["kg","litro","unidad","bolsa","caja","bulto"] else 0, key=f"prod_edit_unidad_stock_{pid_int}")
    c4,c5=st.columns(2)
    with c4: econtenido=st.number_input("Contenido por unidad editable", min_value=0.001, value=float(getattr(row,'contenido_unidad',1) or 1), step=0.5, help="Ej: 1 unidad de aceite X8Lts contiene 8 litros.", key=f"prod_edit_contenido_{pid_int}")
    with c5: emedida=st.selectbox("Medida contenido editable", ["unidad","litro","kg"], index=["unidad","litro","kg"].index(getattr(row,'medida_contenido','unidad')) if getattr(row,'medida_contenido','unidad') in ["unidad","litro","kg"] else 0, key=f"prod_edit_medida_{pid_int}")
    active=st.checkbox("Producto activo", value=bool(row.activo), key=f"prod_edit_activo_{pid_int}")
    c1,c2=st.columns(2)
